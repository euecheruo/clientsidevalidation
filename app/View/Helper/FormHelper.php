...
/**
 * Persistent validation fields used by the form. Set by FormHelper::create().
 *
 * @var array
 * @access protected
 */
	protected $_validationField = array();
 ...
 * @param boolean $clientSideVidation Sets where to use client side validation
 * @return string An formatted opening FORM tag.
 *
 * @modifiedBy: emeka.echeruo@gmail.com
 */
	public function create($model = null, $options = array(), $clientSideValidation = false) {
		$created = $id = false;
		$append = '';
		...
		$this->_validationField[ $options['id'] ]['validators'] = array();
		if($clientSideValidation)
		{
			$this->_validationField[ $options['id'] ]['allowed'] = true;
		} else {
			$this->_validationField[ $options['id'] ]['allowed'] = false;
		}
 ...
 * @param array $links Identifies which forms allow client-side validation
 * @return string a closing FORM tag optional submit button.
 *
 * @modifiedBy: emeka.echeruo@gmail.com
 */
	public function end($options = null, $links = array()) {
		...
		{
			if(array_key_exists($link, $this->_validationField))
			{
				if($this->_validationField[$link]['allowed'])
					$this->Js->get("#" . $link)->validate($this->_validationField[$link]['validators']);
...
/**
 * If forms that use clientside validation
 *
 * @param string $key The form domId
 * @return boolean true or false
 */
	public function clientSideValidation($key)
	{
		array_key_exists($key, $this->_validationField);
		{
			if($this->_validationField[$key]['allowed'])
				return true;
		}
		return false;
	}
 ...
 * @param array $validation options for clientside validation.
 * @return string Completed form widget.
 *
 * @modifiedBy: emeka.echeruo@gmail.com
 */
	public function input($fieldName, $options = array(), $validation = array()) {
		...
		$this->_addValidationRules($elementType, "data[" . $modelKey . "][" . $fieldKey . "]", $validation);
 ...
 * @modifiedBy: emeka.echeruo@gmail.com
 */
	public function checkbox($fieldName, $options = array(), $validation = array()) {
		$valueOptions = array();
		...
		$this->_addValidationRules("input", $options['name'], $validation);
 ...
 * @modifiedBy: emeka.echeruo@gmail.com
 */
	public function radio($fieldName, $options = array(), $attributes = array(), $validation = array()) {
		...
		$this->_addValidationRules("input", $attributes['name'], $validation);
 ...
 * 				Emeka U Echeruo (emeka.echeruo@gmail.com)
 */
	public function textarea($fieldName, $options = array(), $validation = array()) {
		...
		$this->_addValidationRules("textarea", $options['name'], $validation);
...
/**
 * Adds the client-side validation rules
 *
 * @param string $elementType the form element type.
 * @param string $attributeName the value of the name attribute of the element.
 * @param array $validation options for clientside validation.
 * @return void.
 */
	protected function _addValidationRules($elementType, $attributeName, $validation = array())
	{
		$validation = array_merge(array('forms' => array(), 'rules' => array(), 'messages' => array(), 'allow' => true), $validation);
		foreach($validation['forms'] as $link )
		{
			if(array_key_exists($link, $this->_validationField))
			{
				if($this->_validationField[$link]['allowed'] && $validation['allow'])
				{
					$this->_validationField[$link]['validators']['rule'][$elementType . "[name=\"" . $attributeName . "\"]"] = $validation['rules'];
					$this->_validationField[$link]['validators']['message'][$elementType . "[name=\"" . $attributeName . "\"]"] = $validation['messages'];
				}
			}
		}
