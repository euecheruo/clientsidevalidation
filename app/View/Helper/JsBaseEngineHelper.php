<?php
 /**
 * Overrides JsBaseEngineHelper Class.
 * Js Base Engine Helper for JsHelper
 *
 * PHP Version 5
 *
 ...
 */
	protected function _parseOptions($options, $safeKeys = array()) {
		...
		foreach ($options as $key => $value) {
			if(is_array($value))
			{
				$out[] = $key . ':{'  . $this->_parseChildrenOptions($value, $safeKeys) . '}';
			} else {
 ...
 * @return string Parsed JSON options without enclosing { }.
 */
	protected function _parseChildrenOptions($options, $safeKeys = array()) {
